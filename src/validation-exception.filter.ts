...
import { ArgumentsHost, BadRequestException, Catch, ExceptionFilter, HttpException, HttpStatus, WsExceptionFilter } from '@nestjs/common';
...
export class WebSocketExceptionFilter implements WsExceptionFilter {
    ...
    let status = 500;
    let message = 'Something Went Wrong!', errors: any;
      ...
      switch (exception.code) {
        case 11000: // MongoDB duplicate key error code
          ...
          if (exception['keyPattern']) {
            ...
            const matches = exception.message.match(/index: (.+?) dup key: { (.+?) }/);
            if (matches && matches.length > 2) {
              try {
                const [dkey, dvalue] = matches[2].split(':').map(item => item.trim().replace(/['"]/g, ''));
